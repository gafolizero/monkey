...
func (p *Parser) parseIntegerLiteral() ast.Expression {
	...
	if err != nil {
		msg := fmt.Sprintf("cannot convert value in IntegerLiteral %s", value)
		p.errors = append(p.errors, msg)
	}
	integerLiteral.Value = valueInt
...
func (p *Parser) peekPrecedence() int {
	prec, ok := precedence[p.peekToken.Type]
	if !ok {
		return LOWEST
	}
	return prec
...
func (p *Parser) currPrecedence() int {
	prec, ok := precedence[p.currToken.Type]
	if !ok {
		return LOWEST
	}
	return prec
...
func (p *Parser) parseFunctionLiteral() ast.Expression {
	...
	function.Parameters = p.parseFunctionParameter()
